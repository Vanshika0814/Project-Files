--------------------------------------------------------
--  DDL for Package Body XXCNV_GL_I001_GL_JOURNALS_CONVERSION_PKG
--  Thursday 10/30/2025
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY XXCNV.XXCNV_GL_I001_GL_JOURNALS_CONVERSION_PKG IS
	/*************************************************************************************
	...
	-- -------------   ------       ----------  -----------------------------------------
	-- 	Priyanka Kadam	      23-Oct-2025  	    1.0         Initial Development
	--  Satya Pavani 		  11-Nov-2025       1.1         BO-153
	****************************************************************************************/
...
---Declaring global Variables
    ...
    gv_external_table            CONSTANT VARCHAR2(30) := 'xxcnv_gl_i001_gl_interface_ext';
...
==============================================================================================================*/
    ...
    */
    ...
    -- Call to create a CSV file from xxcnv_gl_i001_gl_interface_stg after all validations
        BEGIN
...
===================================================================================================================*/
    ...
    PROCEDURE import_data_from_oci_to_stg_prc (
       ...
       -- lv_oci_file_name VARCHAR2(100) := 'NS_JOURNALS*.csv';
        lv_oci_file_name VARCHAR2(100) := 'GlInterface*.csv';  --v1.1
    ...
    BEGIN
        ...
        -- Check if the external table exists and drop it if it does
            ...
            IF lv_table_count > 0 THEN
                EXECUTE IMMEDIATE 'DROP TABLE xxcnv_gl_i001_gl_interface_ext';
                EXECUTE IMMEDIATE 'TRUNCATE TABLE xxcnv_gl_i001_gl_interface_stg';
                dbms_output.put_line('Table xxcnv_gl_i001_gl_interface_ext dropped');
            END IF;

        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error dropping table xxcnv_gl_i001_gl_interface_ext: '
                                     || '->'
                                     || substr(sqlerrm, 1, 3000)
                                     || '->'
                                     || dbms_utility.format_error_backtrace);
        ...
        -- Create the external table
           -- lv_oci_file_name := 'NS_JOURNALS*.csv';
              lv_oci_file_name := 'GlInterface*.csv';  --v1.1

			
            dbms_cloud.create_external_table(
                table_name      => 'xxcnv_gl_i001_gl_interface_ext',
            ...
            EXECUTE IMMEDIATE 'INSERT INTO xxcnv_gl_i001_gl_interface_stg (
						status,                        
						ledger_id, 
						accounting_date,
						user_je_source_name, 
                        ...
                        null,
						null
						FROM xxcnv_gl_i001_gl_interface_ext';
            dbms_output.put_line('Inserted records in the xxcnv_gl_i001_gl_interface_stg from OCI Source Folder: ' || SQL%rowcount);
        ...
        -- Count the number of rows in the external table
            ...
            INTO lv_row_count
            FROM
                xxcnv_gl_i001_gl_interface_stg;

        -- Use an implicit cursor in the FOR LOOP to iterate over distinct group_ids
     --FOR rec IN (SELECT DISTINCT group_id FROM xxcnv_gl_i001_gl_interface_stg where file_reference_identifier = gv_execution_id) LOOP
      ...
      -- END LOOP;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error counting rows in xxcnv_gl_i001_gl_interface_stg: '
                                     || '->'
                                     || substr(sqlerrm, 1, 3000)
                                     || '->'
                                     || dbms_utility.format_error_backtrace);
...
===================================================================================================================*/
  ...
  /*CURSOR cur_amount_check IS
		...
		FROM xxcnv_gl_i001_gl_interface_stg
    ...
    BEGIN
        BEGIN
            UPDATE xxcnv_gl_i001_gl_interface_stg
            SET
        ...
        BEGIN
            ...
            THEN
                dbms_output.put_line('No Data is found in the xxcnv_gl_i001_gl_interface_stg Table');
                RETURN;
            END IF;
...
-- Group_Id --
  ...
  ---ATTRIBUTE1
        BEGIN
            UPDATE xxcnv_gl_i001_gl_interface_stg
            SET
                attribute_category = 'NETSUITE_INTEGRATION'
            WHERE
                    1 = 1
                AND file_reference_identifier IS NULL;
  ...
  ---PERIOD_NAME
    ...
    /* Ledger Name logic */
        BEGIN
            UPDATE xxcnv_gl_i001_gl_interface_stg gis
            SET
                ledger_name = (
                    SELECT
                        CASE
	...
	--- ledger currency --
        BEGIN
            UPDATE xxcnv_gl_i001_gl_interface_stg stg
            SET
                ledger_currency = (
                    SELECT
                        currency_code
            ...
            SET
                ...
                entered_cr =
                    CASE
                        WHEN currency_code != ledger_currency
                             AND entered_dr > 0
                             AND (accounted_cr > 0 OR accounted_cr IS NOT NULL) THEN
                           - (entered_dr)
                        ELSE
                            entered_cr
                    END;

            dbms_output.put_line('ENTERED_DR AND ENTERED_CR fields are updated when amounts fields are not coming properly');
        END;
                ...
                entered_dr =
                    CASE
                        WHEN currency_code != ledger_currency
                             AND entered_dr > 0
                             AND ( accounted_cr > 0
                                   OR accounted_cr IS NOT NULL ) THEN
                            NULL
                        ELSE
                            entered_dr
                    END;
...
*/
      ...
      -- Updating constant values --
        ...
        BEGIN
            ...
            SET
                status = 'NEW',
                user_je_source_name = 'NETSUITE INTEGRATION',
                actual_flag = 'A',
                reference7 = 'N';
...
================================================================================================================================= */
    PROCEDURE coa_target_segments_prc IS
        ...
        SELECT
            ...
            x.*
        FROM
            xxcnv_gl_i001_gl_interface_stg x
        WHERE
            execution_id = gv_execution_id;
...
================================================================================================================================= */
    ...
    PROCEDURE load_balancing_line_prc IS
        ...
        v_reference4        xxcnv_gl_i001_gl_interface_stg.reference4%TYPE;
        v_diff_accounted_cr xxcnv_gl_i001_gl_interface_stg.accounted_cr%TYPE;
        v_diff_accounted_dr xxcnv_gl_i001_gl_interface_stg.accounted_dr%TYPE;
        lv_diff_amt         NUMBER;
    ...
    BEGIN
        ...
        FOR g_ln IN line_cursor LOOP
            ...
            IF g_ln.sum_accounted_cr <> g_ln.sum_accounted_dr THEN
                ...
                IF ( g_ln.sum_accounted_cr > g_ln.sum_accounted_dr ) THEN
                    INSERT INTO xxcnv_gl_i001_gl_interface_stg (
                        status,
                        accounting_date,
                        user_je_source_name,
                        user_je_category_name,
			...
			/* START added for v1.2*/
                  ...
                  /*  SELECT
                    ...
                    dbms_output.put_line('lv_file: ' || lv_file);*/
				                  ...
				                  -- lv_file := 'GlInterface_'||lv_group_id||'_'||gv_oci_file_name;
				                     lv_file := 'GlInterface_'||lv_group_id; -- v1.1
...
END xxcnv_gl_i001_gl_journals_conversion_pkg;
