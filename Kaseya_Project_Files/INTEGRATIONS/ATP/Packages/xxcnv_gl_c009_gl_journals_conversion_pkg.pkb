create or replace PACKAGE BODY  xxcnv.xxcnv_gl_c009_gl_journals_conversion_pkg IS
	/*************************************************************************************
...
---Declaring global Variables
    ...
    gv_external_table            CONSTANT VARCHAR2(30) := 'xxcnv_gl_c009_gl_interface_ext';
...
==============================================================================================================*/
    ...
    */
    ...
    -- Call to create a CSV file from xxcnv_gl_c009_gl_interface_stg after all validations
        BEGIN
    ...
    BEGIN
        ...
        -- Check if the external table exists and drop it if it does
            ...
            IF lv_table_count > 0 THEN
                EXECUTE IMMEDIATE 'DROP TABLE xxcnv_gl_c009_gl_interface_ext';
                EXECUTE IMMEDIATE 'TRUNCATE TABLE xxcnv_gl_c009_gl_interface_stg';
                dbms_output.put_line('Table xxcnv_gl_c009_gl_interface_ext dropped');
            END IF;

        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error dropping table xxcnv_gl_c009_gl_interface_ext: '
                                     || '->'
                                     || substr(sqlerrm, 1, 3000)
                                     || '->'
                                     || dbms_utility.format_error_backtrace);
        ...
        -- Create the external table
            ...
            dbms_cloud.create_external_table(
                table_name      => 'xxcnv_gl_c009_gl_interface_ext',
            ...
            EXECUTE IMMEDIATE 'INSERT INTO xxcnv_gl_c009_gl_interface_stg (
						status,                        
						ledger_id, 
						accounting_date,
						user_je_source_name, 
                        ...
                        null,
						null
						FROM xxcnv_gl_c009_gl_interface_ext';
            dbms_output.put_line('Inserted records in the xxcnv_gl_c009_gl_interface_stg from OCI Source Folder: ' || SQL%rowcount);
        ...
        -- Count the number of rows in the external table
            ...
            INTO lv_row_count
            FROM
                xxcnv_gl_c009_gl_interface_stg;

        -- Use an implicit cursor in the FOR LOOP to iterate over distinct group_ids
     --FOR rec IN (SELECT DISTINCT group_id FROM xxcnv_gl_c009_gl_interface_stg where file_reference_identifier = gv_execution_id) LOOP
      ...
      -- END LOOP;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error counting rows in xxcnv_gl_c009_gl_interface_stg: '
                                     || '->'
                                     || substr(sqlerrm, 1, 3000)
                                     || '->'
                                     || dbms_utility.format_error_backtrace);
...
===================================================================================================================*/
  ...
  /*CURSOR cur_amount_check IS
		...
		FROM xxcnv_gl_c009_gl_interface_stg
    ...
    BEGIN
        BEGIN
            UPDATE xxcnv_gl_c009_gl_interface_stg
            SET
        ...
        BEGIN
            ...
            THEN
                dbms_output.put_line('No Data is found in the xxcnv_gl_c009_gl_interface_stg Table');
                RETURN;
            END IF;
...
-- Group_Id --
  ...
  ---PERIOD_NAME
    ...
    /* Ledger Name logic */
        BEGIN
            UPDATE xxcnv_gl_c009_gl_interface_stg gis
            SET
                ledger_name = (
                    SELECT
                        CASE
	...
	--- ledger currency --
        BEGIN
            UPDATE xxcnv_gl_c009_gl_interface_stg stg
            SET
                ledger_currency = (
                    SELECT
                        currency_code
...
================================================================================================================================= */
    PROCEDURE coa_target_segments_prc IS
        ...
        SELECT
            ...
            x.*
        FROM
            xxcnv_gl_c009_gl_interface_stg x
        WHERE
            execution_id = gv_execution_id;
...
================================================================================================================================= */
    ...
    PROCEDURE load_balancing_line_prc IS
        ...
        v_reference4        xxcnv_gl_c009_gl_interface_stg.reference4%TYPE;
        v_diff_accounted_cr xxcnv_gl_c009_gl_interface_stg.accounted_cr%TYPE;
        v_diff_accounted_dr xxcnv_gl_c009_gl_interface_stg.accounted_dr%TYPE;
        lv_diff_amt         NUMBER;
    ...
    BEGIN
        ...
        FOR g_ln IN line_cursor LOOP
            ...
            IF g_ln.sum_accounted_cr <> g_ln.sum_accounted_dr THEN
                ...
                IF ( g_ln.sum_accounted_cr > g_ln.sum_accounted_dr ) THEN
                    INSERT INTO xxcnv_gl_c009_gl_interface_stg (
                        status,
                        accounting_date,
                        user_je_source_name,
                        user_je_category_name,
...
END xxcnv_gl_c009_gl_journals_conversion_pkg;