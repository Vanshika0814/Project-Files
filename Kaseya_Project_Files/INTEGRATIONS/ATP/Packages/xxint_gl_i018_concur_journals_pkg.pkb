--------------------------------------------------------
--  DDL for Package Body XXINT_GL_I018_CONCUR_JOURNALS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXINT"."XXINT_GL_I018_CONCUR_JOURNALS_PKG" AS
/********************************************************************************************
OBJECT NAME: GL Concur Expense Journals Package
DESCRIPTION: Package specification for GL_I018
Version 	Name              	Date           		Version-Description
---------------------------------------------------------------------------
<1.0>		Devishi   			23-May-2025 	    1.0-Initial Draft
<1.1> 		Devishi   			17-Sept-2025 	    1.1-Code Optimization
**********************************************************************************************/
	...
	******************************************************************/
    ...
    PROCEDURE validation_transformation_prc (
        ...
        FROM
            xxint_gl_i018_concur_journals_stg
        WHERE
        ...
        SELECT
            ...
            stg.effective_date_of_transaction sent_date,
            stg.rowid                         row_id
        FROM
		----<1.1> Code changes start
			...
			----<1.1> Code changes start
            xxint_gl_i018_concur_journals_stg stg
        WHERE
        ...
        WHERE
            ...
            AND journal_category = 'Card Expense'
    ...
    BEGIN
        BEGIN
            UPDATE xxint_gl_i018_concur_journals_stg
            SET
                status = 'INVALID',
                message = 'Invalid Date Format for Sent Date. Please use YYYY/MM/DD date format'
            WHERE
     ...
     ----Update ledger fields----
        BEGIN
            ...
            UPDATE xxint_gl_i018_concur_journals_stg stg
            SET
                status = 'INVALID',
...
END xxint_gl_i018_concur_journals_pkg;
