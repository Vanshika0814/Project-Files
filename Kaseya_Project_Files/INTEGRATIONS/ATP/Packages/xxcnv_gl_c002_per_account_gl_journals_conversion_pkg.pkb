create or replace PACKAGE BODY  xxcnv.xxcnv_gl_c002_per_account_gl_journals_conversion_pkg IS
	/*************************************************************************************
	...
	--  Priyanka Kadam    04-Aug-2025       1.2         LTCI-6637_Commented the Query as the CVR is Disabled
	--  Satya Pavani      07-Aug-2025       1.3         LTCI-6872 Code updated to work for NONE files
    --  Satya Pavani      25-Aug-2025       1.4         LTCI-7741 - FBDI File name generation change     
	****************************************************************************************/
...
---Declaring global Variables
    ...
    gv_external_table            CONSTANT VARCHAR2(50) := 'XXCNV_GL_C002_PA_GL_INTERFACE_EXT';
...
==============================================================================================================*/
    ...
    -- Call to create a CSV file from xxcnv_gl_c002_pa_gl_interface_stg after all validations
        BEGIN
    ...
    BEGIN
        ...
        -- Check if the external table exists and drop it if it does
            ...
            IF lv_table_count > 0 THEN
                EXECUTE IMMEDIATE 'DROP TABLE xxcnv_gl_c002_pa_gl_interface_ext';
                EXECUTE IMMEDIATE 'TRUNCATE TABLE xxcnv_gl_c002_pa_gl_interface_stg';
                dbms_output.put_line('Table xxcnv_gl_c002_pa_gl_interface_ext dropped');
            END IF;

        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error dropping table xxcnv_gl_c002_pa_gl_interface_ext: '
                                     || '->'
                                     || substr(sqlerrm, 1, 3000)
                                     || '->'
                                     || dbms_utility.format_error_backtrace);
        ...
        -- Create the external table
            ...
            dbms_cloud.create_external_table(
                table_name      => 'xxcnv_gl_c002_pa_gl_interface_ext',
                ...
                column_list     => '
						...
						Transaction_Memo  VARCHAR2(100),
                        period_name VARCHAR2(15),
						File_Amount NUMBER'
            );

            EXECUTE IMMEDIATE 'INSERT INTO xxcnv_gl_c002_pa_gl_interface_stg (
						...
						FROM xxcnv_gl_c002_pa_gl_interface_ext';
            dbms_output.put_line('Inserted records in the xxcnv_gl_c002_pa_gl_interface_stg from OCI Source Folder: ' || SQL%rowcount
            );
        ...
        -- Count the number of rows in the external table
            ...
            INTO lv_row_count
            FROM
                xxcnv_gl_c002_pa_gl_interface_stg;

        -- Use an implicit cursor in the FOR LOOP to iterate over distinct group_ids
     --FOR rec IN (SELECT DISTINCT group_id FROM xxcnv_gl_c002_pa_gl_interface_stg where file_reference_identifier = gv_execution_id) LOOP
      ...
      -- END LOOP;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error counting rows in xxcnv_gl_c002_pa_gl_interface_stg: '
                                     || '->'
                                     || substr(sqlerrm, 1, 3000)
                                     || '->'
                                     || dbms_utility.format_error_backtrace);
...
===================================================================================================================*/
  ...
  -- Declaring Local Variables for validation.
        ...
        ln_file_type        NUMBER;
        ln_subsidiary_id    NUMBER;
        ln_year             NUMBER;
        lv_file             VARCHAR2(200) := NULL;
    BEGIN
        BEGIN
            UPDATE xxcnv_gl_c002_pa_gl_interface_stg
            SET
        ...
        BEGIN
            ...
            IF lv_row_count = 0 THEN
                dbms_output.put_line('No Data is found in the xxcnv_gl_c002_pa_gl_interface_stg Table');
                RETURN;
            END IF;
        EXCEPTION
            WHEN OTHERS THEN
...
---REFERENCE1
        ...
        BEGIN
 --reference 1 and 2 is concatenated
            UPDATE xxcnv_gl_c002_pa_gl_interface_stg
            SET
                reference1 = (
                    SELECT
...
-- Group_Id --
    BEGIN
    UPDATE xxcnv_gl_c002_pa_gl_interface_stg 
    SET group_id = (SELECT  TO_CHAR(SYSDATE,'YYYYMMDDHHMMSS') FROM DUAL)
  ...
  ---PERIOD_NAME
        ...
        BEGIN
            UPDATE xxcnv_gl_c002_pa_gl_interface_stg
            SET
                accounting_date = last_day(TO_DATE(period_name, 'YYYY-MM-DD')),
                date_created = last_day(TO_DATE(period_name, 'YYYY-MM-DD'))
            WHERE
                file_reference_identifier IS NULL;

            dbms_output.put_line('EFFECTIVE_DATE_OF_TRANSACTION and DATE_CREATED is validated');
        END;
	...
	-- Amount check --
        ...
        BEGIN
            UPDATE xxcnv_gl_c002_pa_gl_interface_stg
            SET
                entered_cr = round(entered_cr, 2),
                entered_dr = round(entered_dr, 2);
  ...
  -- Calculate the sum of Entered Credit and Debit
        BEGIN
            SELECT
                NVL(SUM(entered_cr),0)  -- Updated for v1.4
            ...
            SELECT
                NVL(SUM(entered_dr),0)  -- Updated for v1.4
            INTO lv_total_debit
            FROM
                xxcnv_gl_c002_pa_gl_interface_stg
            WHERE
    ...
    -- Check if the sums are equal 
            IF ( lv_total_debit - lv_total_credit ) = 0 THEN
   ...
   --WHERE upper(USER_JE_CATEGORY_NAME) = UPPER('Currency Revaluation');
      ...
      -- Updating constant values --
        ...
        BEGIN
            ...
            SET
                ...
                reference7 = 'N',
                currency_code = 'USD',
                user_je_category_name = 'Period Activity',
                currency_conversion_rate = '1'
			--,LEDGER_NAME = 'PER-ACCOUNT'  --commented for v1.3
                ,
	...
	/* Start added for v1.3 */
        BEGIN
		
            UPDATE xxcnv_gl_c002_pa_gl_interface_stg gis
            SET
                ledger_name = (
                    SELECT
                        CASE
                            WHEN gv_oci_file_name LIKE '%PA%' THEN
                                'PER-ACCOUNT'
                            WHEN gv_oci_file_name LIKE '%NN%' THEN
                                'NONE'
                        END
                    FROM
                        dual
            ...
            dbms_output.put_line('Ledger_name is updated based on the file_name');
	...
	--- ledger currency --
        BEGIN
            UPDATE xxcnv_gl_c002_pa_gl_interface_stg stg
            SET
                ledger_currency = (
                    SELECT
                        currency_code
    ...
    BEGIN
        FOR rec IN (
            SELECT
                ...
                x.*
            FROM
                xxcnv_gl_c002_pa_gl_interface_stg x
            WHERE
                execution_id = gv_execution_id
        ) LOOP
            BEGIN
...
================================================================================================================================= */
    ...
    PROCEDURE load_balancing_line_prc IS
        ...
        SELECT
            ...
            ledger_currency,
            execution_id,
            NVL(SUM(entered_dr),0) AS sum_entered_dr,  
            NVL(SUM(entered_cr),0) AS sum_entered_cr
        FROM
            xxcnv_gl_c002_pa_gl_interface_stg
        WHERE
        ...
        v_reference4        xxcnv_gl_c002_pa_gl_interface_stg.reference4%TYPE;
        v_diff_accounted_cr xxcnv_gl_c002_pa_gl_interface_stg.accounted_cr%TYPE;
        v_diff_accounted_dr xxcnv_gl_c002_pa_gl_interface_stg.accounted_dr%TYPE;
        lv_diff_amt         NUMBER;
        ...
        SELECT
            ...
            reference5,
            segment1,
            NVL(SUM(entered_cr),0) sum_entered_cr,
            NVL(SUM(entered_dr),0) sum_entered_dr
        FROM
            xxcnv_gl_c002_pa_gl_interface_stg
        WHERE
            execution_id = gv_execution_id
    ...
    BEGIN
        ...
        FOR g_ln IN line_cursor LOOP
            lv_diff_amt := 0;
            IF g_ln.sum_entered_dr <> g_ln.sum_entered_cr THEN
                lv_diff_amt := abs(g_ln.sum_entered_cr - g_ln.sum_entered_dr);
                IF ( g_ln.sum_entered_cr > g_ln.sum_entered_dr ) THEN
                    INSERT INTO xxcnv_gl_c002_pa_gl_interface_stg (
                        status,
                        accounting_date,
                        user_je_source_name,
                        user_je_category_name,
        ...
        IF lv_error_count = 0 THEN
			    ...
			    /* START added for v1.4*/
                    ...
                    SELECT
                        ledger_name,
						group_id,
                        TRIM(replace(replace(replace(reference2,'"',''),',',''),' ',''))
                    INTO
                        lv_ledger,
						lv_group_id,
                        lv_subs_name
                    ...
                    query           => 'SELECT 
                                    ...
                                    FROM xxcnv_gl_c002_pa_gl_interface_stg
                                    WHERE import_status = '''
                             || 'PROCESSED'
                             || '''
									AND execution_id  = '''
...
--GL-1--
	...
	/*--GL-5 Product line--      ---v1.2 Commented the Query as the CVR is Disabled
	BEGIN

		UPDATE xxcnv_gl_c002_pa_gl_interface_stg
		SET    error_message = 'CVR Rule GL-5 Account range  400000-499998 cannot use 9999 for Product Line is violated'
			,import_status = 'ERROR'
			,file_reference_identifier = gv_execution_id||gv_status_failure
		WHERE execution_id = gv_execution_id
		AND target_segment4 BETWEEN 400000 and 499998
		AND target_segment5 = 9999;
				COMMIT;
