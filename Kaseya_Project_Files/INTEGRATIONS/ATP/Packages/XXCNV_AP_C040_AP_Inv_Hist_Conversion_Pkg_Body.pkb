create or replace PACKAGE BODY             XXCNV.XXCNV_AP_C040_AP_INV_HIST_CONVERSION_PKG 
 IS
	/*************************************************************************************
    NAME              :     AP_INVOICE_History_CONVERSION_Package BODY
	...
	-- Bhargavi.K     07-Aug-2025       1.1         Added changes for JIRA LTCI-6875
	-- Bhargavi.K     12-Aug-2025       1.2         Added changes for JIRA LTCI-7097
	****************************************************************************************/
...
---Declaring global Variables
      ...
      gv_oci_file_name_apinvlines          VARCHAR2(2000)  := 'ApInvoiceLinesInterface.csv';
...
END;	
   ...
   -- Call to import data from OCI to Stage table
    ...
    -- Call to create a CSV file from xxcnv_AP_C040_ap_invoices_stg after all validations
    BEGIN
...
BEGIN
    BEGIN
       ...
       -- Check if the external table exists and drop it if it does
        ...
        INTO lv_table_count
        FROM all_objects
        WHERE UPPER(object_name) = 'XXCNV_AP_C040_AP_INVOICES_EXT'
        AND object_type = 'TABLE';

        IF lv_table_count > 0 THEN
            EXECUTE IMMEDIATE 'DROP TABLE XXCNV_AP_C040_AP_INVOICES_EXT';
			 EXECUTE IMMEDIATE 'TRUNCATE TABLE xxcnv_AP_C040_ap_invoices_stg';
            dbms_output.put_line('Table XXCNV_AP_C040_AP_INVOICES_EXT dropped');
        END IF;
		EXCEPTION
        WHEN OTHERS THEN
            dbms_output.put_line('Error dropping table XXCNV_AP_C040_AP_INVOICES_EXT: ' ||  '->'|| SUBSTR (SQLERRM, 1, 3000)|| '->'|| DBMS_UTILITY.format_error_backtrace);
    ...
    BEGIN
        ...
        INTO lv_table_count
        FROM all_objects
        WHERE UPPER(object_name) = 'xxcnv_AP_C040_ap_invoice_lines_stg'
        AND object_type = 'TABLE';

        IF lv_table_count > 0 THEN
            --EXECUTE IMMEDIATE 'DROP TABLE XXCNV_AP_C040_APHistoric_LINES_EXT';
					 EXECUTE IMMEDIATE 'TRUNCATE TABLE xxcnv_AP_C040_ap_invoice_lines_stg';
            --dbms_output.put_line('Table XXCNV_AP_C040_APHistoric_LINES_EXT dropped');
        END IF;
		EXCEPTION
        WHEN OTHERS THEN
            dbms_output.put_line('Error truncating table xxcnv_AP_C040_ap_invoice_lines_stg: ' ||  '->'|| SUBSTR (SQLERRM, 1, 3000)|| '->'|| DBMS_UTILITY.format_error_backtrace);
            p_loading_status := gv_status_failure;
    END;
END;	
...
-- Create the external table
	BEGIN
        ...
        IF gv_oci_file_name_apinv LIKE '%ApInvoicesInterface.csv%' THEN
			...
			dbms_output.put_line('Creating external table XXCNV_AP_C040_AP_INVOICES_EXT');
            dbms_output.put_line(' XXCNV_AP_C040_AP_INVOICES_EXT : '|| gv_oci_file_path||'/'||gv_oci_file_name_apinv);

			DBMS_CLOUD.CREATE_EXTERNAL_TABLE(

		   table_name => 'XXCNV_AP_C040_AP_INVOICES_EXT',
           ...
           column_list => 
			...
			EXECUTE IMMEDIATE  'INSERT INTO xxcnv_AP_C040_ap_invoices_stg (
									...
									FROM XXCNV_AP_C040_AP_INVOICES_EXT ';

            p_loading_status := gv_status_success;
			dbms_output.put_line('Inserted Records in the xxcnv_AP_C040_ap_invoices_stg: ' || SQL%ROWCOUNT);
			--commit;
...
INSERT INTO xxcnv_AP_C040_ap_invoice_lines_stg (
					INVOICE_ID,EXECUTION_ID,BATCH_ID,SOURCE_SYSTEM) (select Invoice_ID,EXECUTION_ID,BATCH_ID,SOURCE_SYSTEM from xxcnv_AP_C040_ap_invoices_stg);
			

			                   p_loading_status := gv_status_success;					
				               dbms_output.put_line('Inserted records in xxcnv_AP_C040_ap_invoice_lines_stg: '||SQL%ROWCOUNT);
 ...
 EXCEPTION
    ...
    END;
	   ...
	   -- Count the number of rows in the external table
        ...
        IF gv_oci_file_name_apinv LIKE '%ApInvoicesInterface%' THEN
            SELECT COUNT(*)
            INTO lv_row_count
            FROM xxcnv_AP_C040_ap_invoices_stg;
            dbms_output.put_line('Inserted Records in the xxcnv_AP_C040_ap_invoices_stg from OCI Source Folder: ' || lv_row_count);
		END IF;	

		IF gv_oci_file_name_apinvlines LIKE '%ApInvoiceLinesInterface%' THEN
            SELECT COUNT(*)
            INTO lv_row_count
            FROM xxcnv_AP_C040_ap_invoice_lines_stg;
            dbms_output.put_line('Inserted Records in the xxcnv_AP_C040_ap_invoice_lines_stg from OCI Source Folder: ' || lv_row_count);
        END IF;	
   ...
   BEGIN
        ...
        dbms_output.put_line('Log:Inserted Records in the xxcnv_AP_C040_ap_invoices_stg from OCI Source Folder: ' || lv_row_count);

		 -- Use an implicit cursor in the FOR LOOP to iterate over distinct book_type_code
        FOR rec IN (SELECT DISTINCT batch_id FROM xxcnv_AP_C040_ap_invoices_stg where execution_id = gv_execution_id) LOOP
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    dbms_output.put_line('Error counting rows in xxcnv_AP_C040_ap_invoices_stg: ' || SQLERRM);
                    p_loading_status := gv_status_failure;
                    RETURN;
            END;  
   ...
   BEGIN
        ...
        dbms_output.put_line('Log:Inserted Records in the xxcnv_AP_C040_ap_invoice_lines_stg from OCI Source Folder: ' || lv_row_count);

		 -- Use an implicit cursor in the FOR LOOP to iterate over distinct book_type_code
        FOR rec IN (SELECT DISTINCT batch_id FROM xxcnv_AP_C040_ap_invoice_lines_stg where execution_id = gv_execution_id) LOOP
                xxcnv_cmn_conversion_log_message_pkg.write_log_prc(
                    p_conversion_id 		=> gv_conversion_id,
                   	p_execution_id		    => gv_execution_id,
                    p_execution_step		=> gv_status_picked,
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    dbms_output.put_line('Error counting rows in xxcnv_AP_C040_ap_invoice_lines_stg: ' || SQLERRM);
                    p_loading_status := gv_status_failure;
                    RETURN;
            END;			
...
BEGIN
 ...
 EXCEPTION
  ...
  END;
	  ...
	  -- Validate Unique Invoice IDs in xxcnv_AP_C040_ap_invoices_stg

-- Step 1: Check for duplicate INVOICE_IDs in xxcnv_AP_C040_ap_invoices_stg
BEGIN

UPDATE xxcnv_AP_C040_ap_invoices_stg
SET ERROR_MESSAGE = ERROR_MESSAGE || '|Duplicate INVOICE_IDs found in xxcnv_AP_C040_ap_invoices_stg. '
WHERE INVOICE_ID IN (
    SELECT INVOICE_ID
    FROM xxcnv_AP_C040_ap_invoices_stg
    WHERE execution_id = gv_execution_id
...
--FUT
-- Step 2: Check for missing INVOICE_IDs in xxcnv_AP_C040_ap_invoice_lines_stg
BEGIN
UPDATE xxcnv_AP_C040_ap_invoices_stg h
SET ERROR_MESSAGE = ERROR_MESSAGE || '|Some INVOICE_IDs in Header Table do not have corresponding entries in Lines Table. '
WHERE NOT EXISTS (
    SELECT 1
    FROM xxcnv_AP_C040_ap_invoice_lines_stg l
...
--  validate Business_Unit
  ...
  Begin
  UPDATE xxcnv_AP_C040_ap_invoices_stg
  SET OPERATING_UNIT='ITG'
  WHERE OPERATING_UNIT in ( 'Datto Canada Enterprises, Inc.', 'DTO - Datto Canada Enterprises, Inc.')
  and execution_id = gv_execution_id 
           AND file_reference_identifier IS NULL;
                dbms_output.put_line('CANADA OPERATING_UNIT is validated');
            END;
 ...
 --  validate Business_Unit after transformation 
   BEGIN
    UPDATE xxcnv_AP_C040_ap_invoices_stg 
    SET ERROR_MESSAGE = ERROR_MESSAGE || '|Corresponding BUSINESS_UNIT should not be NULL'
...
BEGIN
 UPDATE xxcnv_AP_C040_ap_invoices_stg l
 SET ERROR_MESSAGE = ERROR_MESSAGE || '|Header AMOUNT must be equal to Sum of Line Amount'
 where l.INVOICE_AMOUNT != (select sum(h.amount) from  xxcnv_AP_C040_ap_invoice_lines_stg h where h.INVOICE_ID = L.INVOICE_ID
                          AND h.LINE_TYPE_LOOKUP_CODE IN ('ASSEMBLY', 'INVTPART', 'NONINVTPART','TAXITEM', 'TAXGROUP'))
 and file_reference_identifier IS NULL;
 ...
 BEGIN
    UPDATE xxcnv_AP_C040_ap_invoices_stg a
	...
	and file_reference_identifier is NULL;
    dbms_output.put_line('VENDOR_SITE_CODE is updated');
  END;
...
--Validate Supplier Site with comma's
  ...
  BEGIN 
          ...
          WHERE VENDOR_SITE_CODE LIKE '%,%'
           and execution_id = gv_execution_id 
           AND file_reference_identifier IS NULL;
                dbms_output.put_line('Original Supplier site With Comma is validated');
            END;
-- Validate Supplier Site for Null values
		BEGIN
              UPDATE xxcnv_AP_C040_ap_invoices_stg
              SET ERROR_MESSAGE = ERROR_MESSAGE || '|Corresponding Supplier Site Not Found In Oracle'
...
BEGIN
      ...
      UPDATE xxcnv_AP_C040_ap_invoices_stg
        SET group_id = 'AP_HIST_INV_CONV_'|| TO_CHAR(SYSDATE, 'YYYYMMDDHH24MISS')
        WHERE execution_id = gv_execution_id;
...
BEGIN 
 ...
 END;
 */
 -- Added changes for V1.2
   Begin
  UPDATE xxcnv_AP_C040_ap_invoices_stg
  SET legal_entity_name ='ITG'  
  WHERE legal_entity_name in ( 'Datto Canada Enterprises, Inc.', 'DTO - Datto Canada Enterprises, Inc.')
  and execution_id = gv_execution_id 
           AND file_reference_identifier IS NULL;
                dbms_output.put_line('CANADA OPERATING_UNIT is validated');
            END;
...
END;
  ...
  BEGIN 
    ...
    --  AND execution_id = gv_execution_id 
      AND file_reference_identifier IS NULL;
           dbms_output.put_line('TARGET_legal_entity_name With Comma is validated');
 END;

-- Added changes for V1.1
BEGIN 
     UPDATE xxcnv_AP_C040_ap_invoices_stg 
     SET legal_entity_name = '"' || (replace( legal_entity_name,'"','') )  || '"' 
     WHERE legal_entity_name LIKE '%,%'  
...
UPDATE xxcnv_AP_C040_ap_invoice_lines_stg
          SET execution_id = gv_execution_id
		  WHERE file_reference_identifier is null;
...
END;
   ...
   BEGIN                                                                                                                      
 Update xxcnv_AP_C040_ap_invoice_lines_stg l
SET 
	l.line_number = 1,
	l.LINE_TYPE_LOOKUP_CODE = 'ITEM',
	l.amount = 0,
	l.accounting_date = sysdate;
--	TRUNC(ADD_MONTHS(SYSDATE, 1), 'MM');
...
-- DIST_CODE_CONCATENATED Validation
 ...
 BEGIN                                                                                                                      
 Update xxcnv_AP_C040_ap_invoice_lines_stg l
SET l.DIST_CODE_CONCATENATED = 
(SELECT (substr(ACCTS_PAY_CODE_CONCATENATED,1,instr(ACCTS_PAY_CODE_CONCATENATED,'-',1,1)-1))
||('-999-99999-211999-9999-999999-9999-9999-9999-999999')
FROM xxcnv_AP_C040_ap_invoices_stg 
WHERE INVOICE_ID = l.invoice_id);
 ...
 END;
  ...
  BEGIN                                                                                                                      
 Update xxcnv_AP_C040_ap_invoice_lines_stg l
SET l.description = (select description from xxcnv_AP_C040_ap_invoices_stg where INVOICE_ID = l.invoice_id);
 ...
 END;
  ...
  --Erroring out the record in child table as it errored out in parent table
		  BEGIN
              -- Update the import_status in xxcnv_AP_C040_ap_invoice_lines_stg to 'ERROR' where the PARENT RECORD   has import_status 'ERROR'
              UPDATE xxcnv_AP_C040_ap_invoice_lines_stg
    ...
    BEGIN
        ...
        dbms_output.put_line('Creating FBDI file for batch_id: ' || lv_batch_id);
            ...
            BEGIN
                ...
                dbms_output.put_line('Success record count for xxcnv_AP_C040_ap_invoices_stg for batch_id ' || lv_batch_id || ': ' || lv_success_count);

            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    dbms_output.put_line('No data found for xxcnv_AP_C040_ap_invoices_stg for batch_id: ' || lv_batch_id);
                    RETURN; --
                WHEN OTHERS THEN
                    dbms_output.put_line('Error checking success record count for xxcnv_AP_C040_ap_invoices_stg for batch_id ' || lv_batch_id || ': ' || SQLERRM);
                    RETURN; --
            END;
            ...
            IF lv_success_count > 0 THEN
                BEGIN
                    ...
                    dbms_output.put_line('CSV file for xxcnv_AP_C040_ap_invoices_stg for batch_id ' || lv_batch_id || ' exported successfully to OCI Object Storage.');
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        dbms_output.put_line('Error exporting data to CSV for xxcnv_AP_C040_ap_invoices_stg for batch_id ' || lv_batch_id || ': ' || SQLERRM);

                        RETURN;
                END;
           ELSE
                dbms_output.put_line('Process Stopped for xxcnv_AP_C040_ap_invoices_stg for batch_id ' || lv_batch_id || ': Error message columns contain data.');
			RETURN;
            END IF;
            END LOOP;
		...
		BEGIN
			...
			BEGIN
               ...
               -- AND TRIM(error_message) != '';
                ...
                dbms_output.put_line('Success record count for xxcnv_AP_C040_ap_invoice_lines_stg for batch_id ' || lv_batch_id || ': ' || lv_success_count);

            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    dbms_output.put_line('No data found for xxcnv_AP_C040_ap_invoice_lines_stg for batch_id: ' || lv_batch_id);
                    RETURN;
                WHEN OTHERS THEN
                    dbms_output.put_line('Error checking success record count for xxcnv_AP_C040_ap_invoice_lines_stg for batch_id ' || lv_batch_id || ': ' || SQLERRM);
                    RETURN;
            END;
			...
			INVOICE_ID						,
                    ...
                    dbms_output.put_line('CSV file for xxcnv_AP_C040_ap_invoices_stg for batch_id ' || lv_batch_id || ' exported successfully to AP_INVOICE_LINES OCI Object Storage.');
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        dbms_output.put_line('Error exporting data to CSV for xxcnv_AP_C040_ap_invoice_lines_stg for batch_id ' || lv_batch_id || ': ' || SQLERRM);

                        RETURN;
                END;
            ELSE
                dbms_output.put_line('Process Stopped for xxcnv_AP_C040_ap_invoice_lines_stg for batch_id ' || lv_batch_id || ': Error message columns contain data.');
                RETURN;
			END IF;
...
BEGIN
  ...
  SELECT 
        ...
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error exporting data to CSV for xxcnv_AP_C040_ap_invoices_stg for batch_id ' || lv_batch_id || ': ' || '->' || SUBSTR(SQLERRM, 1, 3000) || '->' || DBMS_UTILITY.format_error_backtrace);
                RETURN;
        END;
    END LOOP;
...
BEGIN
  FOR g_id IN batch_id_cursor_lines LOOP
       ...
       SELECT 
            ...
            dbms_output.put_line('CSV file for xxcnv_AP_C040_ap_invoice_lines_stg for batch_id ' || lv_batch_id || ' exported successfully to OCI Object Storage.');
        ...
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error exporting data to CSV for xxcnv_AP_C040_ap_invoice_lines_stg for batch_id ' || lv_batch_id || ': ' || '->' || SUBSTR(SQLERRM, 1, 3000) || '->' || DBMS_UTILITY.format_error_backtrace);
               -- RETURN;
        END;
    END LOOP;
...
BEGIN
    ...
    FOR rec IN (SELECT rowid as identifier, x.* FROM xxcnv_AP_C040_ap_invoices_stg x
